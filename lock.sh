...
OPTIONS="Options:
    ...
    -c, --color      Set a color instead of creating an image
...
TEMP="$(getopt -o :hc:dpglt:f: -l color,desktop,help,listfonts,pixelate,greyscale,text:,font: --name "$0" -- "$@")"
eval set -- "$TEMP"
...
if [ -z "$BGCOLOR" ]; then
	...
	convert -size $(xrandr -q | awk -F'current' -F',' 'NR==1 {gsub("( |current)","");print $2}') xc:"$BGCOLOR" "$IMAGE"
fi
